...
use Psito\Themes\Exceptions\FileMissingException;
...
class Themes
{
	...
	/**
	 * Add bonsai.json file is the Psito Bonsai package
	...
	{
		if (class_exists('Psito\Bonsai\Bonsai')) {
			$activeTheme = $this->getActive();
			$path        = $this->getPath();
			$parent      = $this->getProperty($activeTheme.'::parent');
			$themePath   = $path.'/'.$theme;
	...
	{
		if (class_exists('Psito\Modules\Modules')) {
